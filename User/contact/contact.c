...
void LeftMovingSpeedW(unsigned int val)//左轮方向和速度控制函数
{
    if(val>10000)
    {
        ...
        Control_left.OwenValue=(val-10000);//PID调节的目标编
    }
    else if(val<10000)
    {
        ...
        Control_left.OwenValue=(10000-val);//PID调节的目标编    数
    }
    else
    {
         GPIO_SetBits(GPIOC, GPIO_Pin_6);
...
void RightMovingSpeedW(unsigned int val2)//右轮方向和速度控制函数
{
    ...
    else if(val2<10000)
    {
        ...
        Control_right.OwenValue=(10000-val2);//PID调节的目标编    数
    }
    else
    {
        GPIO_SetBits(GPIOC, GPIO_Pin_10);
