...
extern u16 USART_RX_STA;                   //串口接收状态    记
...
int main(void)
{
    ...
    while (1)
    {
        if(main_sta&0x01)//执行发送里程计数据步骤
        {
            ...
            for(i=0;i<21;i++)
            {
                USART_ClearFlag(USART1,USART_FLAG_TC);  //在发送第一个数据前加此句，解决第一个数据不能                的问题
                USART_SendData(USART1,odometry_data[i]);//发送一个字节到    口
                while(USART_GetFlagStatus(USART1, USART_FLAG_TC) == RESET);    //等待发送结
            }
        ...
        car_control(rightdata.d,leftdata.d);     //将接收到的左右轮速度    给小车
    }//end_while
}//end main
/*********************************************END OF FILE**************************************************/
